...
models <- by(df_merged, df_merged$Model, function(subdata) {
  model <- lm(mean_relatedness ~ Distance, data = subdata)
...
results %>%
  ...
  theme(text = element_text(size = 15),
        legend.position="bottom") +
  labs(x = "Residuals (Relatedness ~ Distance)") +
  facet_wrap(~Model)
```
